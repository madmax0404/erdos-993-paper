...
coefficientwise Schur lemma; at uniform multisets it is certified on
the finite range used here by the factorial ladder criterion, whose
bottom rung is the symmetric pairwise \emph{E-core}.  We also certify
the E-core on a large vertex-background range by an exact polynomial
method and prove its all-$E$ limiting form by a five-line calculus
...
failure, an observed offset law for the location of log-concavity failures
in the searched uniform hub spiders, and the observation that the
...
The proof of Theorem~\ref{thm:scope} is a composition of manuscript
lemmas with exact-script-verified components, all executed in exact
...
\begin{equation}\label{eq:sym}
  ...
  \qquad W=\prod_{l\ne i,j}A_l,
\end{equation}
This is the symmetric flow form.  The E-core certificates below address
its all-$E$ pair-slot, vertex-background specializations; the finite
uniform flow statement is proved instead by the factorial ladder
criterion.
...
The following results establish the two-hub case and the certified
uniform range used in the unconditional scope.

\begin{theorem}[E-core at $h=2$]\label{thm:h2}
For equal arms at $h=2$, the core inequality
\[
  [b^2]_k[E^2]_k\le 2[bE]_k^2
\]
holds for all $c\ge1$ and all $k\ge0$, with no band restriction.
\end{theorem}
...
For uniform multisets $S(c^h)$, the factorial ladder
\eqref{eq:ladder}, hence the flow concavity \eqref{eq:flowcc}, holds
for all integer $c\ge1$ at every $3\le h\le10$, every $k\le28$, and
every rung; this is the $p=h$ subfamily of the $7{,}866$
two-adjacent-value polynomial certificates of
Section~\ref{sec:verification}.  Separately, the E-core all-$E$
pair-slot specialization of \eqref{eq:sym} is certified for all
integer $c\ge1$ at the $996$ vertex-background instances with
$3\le h\le7$, $k\le28$ and $8\le h\le10$, $k\le20$.
\end{theorem}

Finally, the counterexample search that initiated this work produced
extremal information of independent interest
(Section~\ref{sec:extremal}): an observed \emph{offset law} in the
uniform hub-spider range $h\le25$, $c\le12$, where for $c\ge4$ the
unique log-concavity failure sits exactly $h-2$ positions below the
top degree, and high-fragility junction trees with balance about
$0.045$, still far below what a unimodality counterexample requires.
...
inequality \eqref{eq:flowcc} of Theorem~\ref{thm:reduction}.  At
uniform multisets this concavity is certified here by the factorial
ladder \eqref{eq:ladder}, whose bottom rung is the all-$E$ symmetric
E-core specialization of \eqref{eq:sym}.  The final assembly into
explicit certificates, verified in exact rational arithmetic, is described in
Section~\ref{sec:verification}.
...
all-$E$ symmetric E-core specialization of \eqref{eq:sym}.  Each
bracket $u_s$ is a polynomial in $c$ at fixed $(k,h,s)$, so the higher
rungs are certifiable by the method of Section~\ref{sec:polyc}.  Exact scans
...
This section proves the core two-slot inequalities used in the
two-hub case and records the pair-slot expansion behind the E-core
certificates.  No vertex reduction is used over background flow
variables: for a background slot the symmetric-form ratio is generally
quadratic-over-quadratic and may have an interior maximum.  The
finite uniform scope is therefore certified by the factorial ladder
of Proposition~\ref{prop:ladder}, not by extending vertex-background
E-core certificates to arbitrary background flows.
...
It suffices to prove the core inequality
...
The finite-$c$ statements in Theorem~\ref{thm:cert} are proved by
exact polynomial certificate scripts: the $p=h$ factorial-ladder
subfamily for uniform flow concavity and, separately, the
vertex-background E-core certificates described in
Section~\ref{sec:verification}.

\section{Certificates and exact-script verification}\label{sec:verification}

This section describes the exact-script-verified components and the
...
statement.  The accompanying repository also contains a Lean
formalization of the Taylor-shift positivity checker and the generated
vertex-background E-core polynomial instances, but the manuscript is not
claimed to be fully theorem-prover verified.
...
At fixed $(k,h)$ and vertex-background pattern
$W=E^{h-2-w}(E+2b)^w$ in the uniform case, each bracket of
\eqref{eq:sym} is a polynomial in $c$ of degree at most $k$ (each
factor coefficient is a polynomial in $c$; the bracket is a finite
convolution).  The claim ``\eqref{eq:sym} holds for \emph{all
integers} $c\ge1$'' is therefore decidable exactly: write
$R(c)=h\,[bEW]_k^2-(h-1)[b^2W]_k[E^2W]_k$, check the leading
coefficient is positive, choose $B$ at least the largest real root
(any upper bound suffices; we use a numerically located bound, then
\emph{prove} positivity beyond it by verifying that all coefficients
of the Taylor shift $R(c+B)$ are nonnegative), and evaluate $R$
exactly at every integer in $[1,B]$.  The numeric root location plays
no role in the rigor; it only sizes the verified range.  This
certified the $996$ vertex-background E-core instances of
Theorem~\ref{thm:cert} with zero failures.  These certificates are
not used to control arbitrary background flow variables.
...
unchanged.  A later review found a false background-flow vertex
reduction lemma; the current manuscript removes that lemma and routes
the uniform proof through the independently certified factorial
ladder.  The audit report and all scripts are in the accompanying
repository.
...
symmetric-form ratio (zero mismatches).  The proof of hypothesis (ii)
for the listed uniform families, however, uses the certified factorial
ladder and Proposition~\ref{prop:ladder}; it does not use a
background-flow vertex reduction.
...
For $h=2$, Theorem~\ref{thm:h2mixed} supplies hypothesis (ii) for
all arm pairs, including equal arms.  Hypothesis (i) is verified for
every $h=2$ multiset in the certificate grid, and band B is empty:
$k_{\mathrm{dec}}\le\lceil(2C+3)/3\rceil\le C$ for $C\ge3$.  Thus
Theorem~\ref{thm:reduction} applies to the equal-arm two-hub family
and to the eleven mixed two-hub members.  For the sixteen three-hub
adjacent-two-value families, hypothesis (ii) is supplied by the
ladder certificates on $(a^p,(a{+}1)^q)$
...
except at $(1,1,2)$, whose single position is checked directly.  For
the listed uniform families with $3\le h\le10$,
Theorem~\ref{thm:cert} supplies the full factorial ladder at every
band position because $k_A\le28$ exactly for the table of
Theorem~\ref{thm:scope}.  Proposition~\ref{prop:ladder} therefore
yields hypothesis (ii), hypothesis (i) is verified by the certificate
grid, and Theorem~\ref{thm:reduction} applies.
\end{proof}
...
balance $>1$ is the width-one special case.  It is the operative
diagnostic for the hub-spider and junction searches below, although
other tree families can have several consecutive log-concavity breaks
\cite{BautistaRamos}.
\end{definition}
...
\paragraph{The observed offset law.}
...
\paragraph{High-fragility trees.}
...
produced a concrete high-fragility seed: a $48$-vertex tree (root,
four $H_4$ hub gadgets, one $11$-vertex gadget) with a single
log-concavity failure of balance $0.04512$ --- more than double the
previous record ($0.0208$, for the family $T^*$).  It is the first
non-log-concave state in the seeded beam log; one $59$-vertex
extension reaches balance $0.04554$, after which the best logged
extensions degrade toward disappearance of the dip.  The $48$-vertex
witness has non-monic sequence (leading coefficient $2$), which did
not occur among the cited Kadrawi--Levit examples.
...
here because the structural caps (the observed offset law, the Maclaurin
tension, the decreasing-zone protection) are proof-shaped statements
that delimit where a counterexample by this junction mechanism would
have to live: below the decreasing zone or outside the width-one local
balance regime.
...
The author used AI tools, including Claude (Anthropic) and ChatGPT/Codex
(OpenAI), during proof exploration, drafting, code development, audit, and
reproducibility packaging.  These tools are not authors.  This draft is being
prepared for independent mathematical review; for any submitted version, the
human author is responsible for the manuscript, proofs, verification code and
data, and final approval.  All verification scripts and paper-facing logs are
